...
data_uk_wide <- data_uk %>% select(-c("cohort_born", "unemployment_rate"))
...
where <- data_uk %>% ungroup() %>% filter(age %in% c(50:65)) %>% select(-c("orgpid", "age", "cohort_born", "unemployment_rate"))
...
for (i in c(1:n_imputations)) {
  ...
  data_uk_imp <- complete(imputation, 1)
  ...
  # sort variables
  data_uk_imp <- data_uk_imp %>% select("age", "retired", "homeowner", "homeowner_type", "female", "year_born", "cohort_born", "year", "migrant", "education_level", "poor_health", "marital_status", "partner_in_hh", "children_any", "household_size", "employment_status", "occupation_group", "public_sector", "self_employed", "hh_income_eq", "unemployment_rate", "region")
...
```{r}
save.image("data/data_uk_final.RData")
