...
# hh_income_eq
data_cpf <- data_cpf %>%
  mutate(hh_income_eq = log((hhinc_post + 1) / sqrt(household_size))) %>%
  select(-hhinc_post)
...
infl <- read.xlsx("data/data_raw/inflation_rates.xlsx")
...
data_cpf <- data_cpf %>% select(country, orgpid, hid, age, year, female, year_born, cohort_born, retired, migrant, education_level, marital_status, partner_in_hh, children_any, household_size, poor_health_sr, poor_health_sb, employment_status, hh_income_eq, occupation_group, public_sector, self_employed, unemployment_rate, inflation_rate)
